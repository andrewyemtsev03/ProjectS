...
import { getCountryName } from "./countries-data.js?v=worldwide-9";
...
export class GlobeMap {
  ...
  renderLabels() {
    ...
    this.countries.forEach((country) => {
      ...
      name.className = "globe-marker-label__name";
      ...
      button.append(flag, name, pointer);
  ...
  setLanguage(lang) {
    this.lang = selectLabel[lang] ? lang : "en";
    this.labelElements.forEach((label, countryId) => {
      ...
      const name = label.querySelector(".globe-marker-label__name");
      if (name) name.textContent = countryName;
    });
  }
